...
def administracion():
    ...
    return render_template('administracion.html', nombre=nombre)
...
@app.route('/restablecer_contra', methods=['GET', 'POST'])
...
def eliminar_hora():
    ...
    if id:
        hora = session.get(HorasTrabajadas, int(id))
        if hora:
            session.delete(hora)
            session.commit()
    return redirect(url_for('horas_trabajadas'))
...
def inventario():
    if request.method == 'POST':
        ...
        producto_existente = session.query(Producto).filter(Producto.nombre.ilike(nombre)).first()

        if producto_existente:
            producto_existente.cantidad += int(cantidad)
        else:
            nuevo_producto = Producto(nombre=nombre.title(), cantidad=int(cantidad))
...
def agregar_reporte():
    if request.method == 'POST':
        ...
        # Agrega a la sesión y confirma el commit
...
def eliminar_reporte(id):
    reporte = session.get(Reporte, id)
    if reporte:
